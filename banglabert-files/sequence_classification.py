...
def main():
    ...
    if training_args.do_predict:
        ...
        # output_predict_file = os.path.join(training_args.output_dir, f"predictions.csv")

        pred_file = data_args.test_file
        pred_file = pred_file.replace(".csv", "")

        output_predict_file = os.path.join(training_args.output_dir, f"{pred_file}.csv")

        if trainer.is_world_process_zero():
            with open(output_predict_file, "w") as writer:
                logger.info(f"***** Predict results *****")
                #   I added Comma for CSV output
                writer.write("sentence1,label\n")
                for index, item in enumerate(predictions):
                    sentence = predict_dataset[index]["sentence1"]
                    item = label_list[item]
                    writer.write(f"\"{sentence}\",{item}\n")
